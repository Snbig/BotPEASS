...
def generate_new_cve_message(cve_data: dict) -> str:
    ...
    message = f"🚨 [https://nvd.nist.gov/vuln/detail/{cve_data['id']}]({cve_data['id']}) 🚨\n"
...
def send_telegram_message(message: str, public_expls_msg: str):
    ...
    message = message.replace(".", "\\.").replace("-", "\\-").replace("(", "\\(").replace(")", "\\)").replace("_", "\\_").replace("[","\\[").replace("]","\\]").replace("{","\\{").replace("}","\\}").replace("=","\\=").replace("#","\\#")
